...
def generate_coin_card(coin, gdata):
    ...
    drw.text((P+18, P+218), f"${target:,.0f}", font=f22, fill=GREEN)
    ...
    drw.text((mid+26, P+218), f"${stop:,.0f}", font=f22, fill=RED)
    ...
    for idx, (lbl, val, tf) in enumerate([
        ("RSI-6",  coin.get("rsi6",50),  "скальпинг"),
        ("RSI-12", coin.get("rsi12",50), "интрадей"),
        ("RSI-24", coin.get("rsi24",50), "свинг"),
    ]):
        bx = P + idx*(bw2+16)
        rc = rsi_col(val)
        rnd(drw, bx, y1+26, bx+bw2, y1+116, CARD, 12)
        drw.text((bx+16, y1+34), lbl, font=fb, fill=WHITE)
        drw.text((bx+bw2-16-tw(drw,tf,f11), y1+36), tf, font=f11, fill=LGRAY)
        drw.text((bx+16, y1+58), str(val), font=f26, fill=rc)
        bar(drw, bx+16, y1+108, bw2-32, 8, val/100, rc)

    r6  = coin.get("rsi6", 50)
    r12 = coin.get("rsi12", 50)
    r24 = coin.get("rsi24", 50)
    cy2 = y1 + 130
    ...
    if fr is not None:
        ...
        fr_s, fr_c, fri = "N/A", DGRAY, "нет данных"
    ...
    if fg.get("ok"):
        ...
        gvals.append(("FEAR & GREED", f"{fv}/100", fg.get("label",""), fc))
    else:
...
def generate_all_cards(data):
    ...
    for coin in data.get("coins", []):
        try:
            ...
            print(f"Card error {coin.get('symbol','?')}: {e}")
    return cards
